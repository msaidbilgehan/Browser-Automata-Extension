...
describe("executeFlow — click node element resolution", () => {
    ...
    beforeEach(() => {
        ...
        mockFn(chrome.storage.local.get).mockImplementation(async (key) => key in store ? { [key]: store[key] } : {});
        mockFn(chrome.storage.local.set).mockImplementation(async (items) => {
    ...
    it("fails the flow and logs an error when the click selector matches nothing", async () => {
        ...
        const result = await executeFlow("flow-1", 1);
    ...
    it("surfaces the failure on the page via an error toast", async () => {
        mockFn(chrome.scripting.executeScript).mockResolvedValue([{ result: false }]);
        await executeFlow("flow-1", 42);
        expect(chrome.tabs.sendMessage).toHaveBeenCalledWith(42, expect.objectContaining({ type: "SHOW_TOAST", level: "error" }));
    });

...
function blockRule(id, enabled = true) {
    return {
        id,
        name: `rule-${id}`,
...
describe("syncNetworkRules — collision-free numeric rule IDs (C3)", () => {
    ...
    beforeEach(() => {
        ...
        mockFn(chrome.storage.local.get).mockImplementation(async (key) => {
            if (key === "networkRules")
                return { networkRules };
            if (key === "networkRuleIds")
                return ruleIdStore ? { networkRuleIds: ruleIdStore } : {};
            return {};
        });
        mockFn(chrome.storage.local.set).mockImplementation(async (items) => {
            if ("networkRuleIds" in items) {
                ruleIdStore = items["networkRuleIds"];
            }
        });

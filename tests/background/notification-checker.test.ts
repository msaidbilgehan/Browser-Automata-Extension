...
function notificationRule(id: string, enabled = true): Record<string, unknown> {
  return {
    id,
    name: `rule-${id}`,
...
describe("checkNotificationRuleById (C1)", () => {
  let notificationRules: Record<string, unknown>;
  ...
  beforeEach(() => {
    ...
    mockFn(chrome.storage.local.get).mockImplementation(async (key: string) =>
      key === "notificationRules" ? { notificationRules } : {},
    );
...
describe("checkNotificationRuleById — change detection & anti-spam", () => {
  ...
  beforeEach(() => {
    ...
    mockFn(chrome.storage.session.get).mockImplementation(async (key: string) =>
      key in sessionStore ? { [key]: sessionStore[key] } : {},
    );
    mockFn(chrome.storage.session.set).mockImplementation(
  ...
  function textChangesRule(id: string): Record<string, unknown> {
    return {
      id,
      name: `rule-${id}`,
      scope: { type: "global", value: "" },

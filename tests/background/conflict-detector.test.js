...
import { serializeKeyCombo, BROWSER_SHORTCUTS, } from "@/background/services/conflict-detector";
...
describe("serializeKeyCombo", () => {
    ...
    it("orders modifiers Ctrl, Alt, Shift, Meta", () => {
        expect(serializeKeyCombo(combo({ key: "k", ctrlKey: true, altKey: true, shiftKey: true, metaKey: true }))).toBe("Ctrl+Alt+Shift+Meta+K");
    });
    ...
    it("produces a string that matches the browser-shortcut warning list for letter combos", () => {
        ...
        expect(BROWSER_SHORTCUTS.includes(serializeKeyCombo(combo({ key: "s", ctrlKey: true })))).toBe(true);
        expect(BROWSER_SHORTCUTS.includes(serializeKeyCombo(combo({ key: "w", ctrlKey: true })))).toBe(true);
    });
});

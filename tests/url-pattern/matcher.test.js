...
describe("matchUrl — exact matching", () => {
    it("matches when the hostname equals the pattern value", () => {
        expect(matchUrl(exact("github.com"), "https://github.com/user/repo")).toBe(true);
    });
    ...
    it("does NOT match a subdomain of the pattern hostname", () => {
        expect(matchUrl(exact("github.com"), "https://docs.github.com")).toBe(false);
    });
...
describe("matchUrl — glob matching", () => {
    describe("wildcard subdomain (*.host)", () => {
        ...
        it("matches a subdomain with root path only", () => {
            expect(matchUrl(glob("*.github.com/"), "https://docs.github.com/")).toBe(true);
        });
        ...
        it("matches any single-level subdomain with root path", () => {
            expect(matchUrl(glob("*.example.com/"), "https://api.example.com/")).toBe(true);
            expect(matchUrl(glob("*.example.com/"), "https://www.example.com/")).toBe(true);
        });
        it("matches a multi-level subdomain", () => {
            expect(matchUrl(glob("*.example.com/"), "https://a.b.example.com/")).toBe(true);
        });
    });
    describe("wildcard path (host/*)", () => {
        it("matches a single path segment", () => {
            expect(matchUrl(glob("github.com/*"), "https://github.com/user")).toBe(true);
        });
    ...
    describe("double-star glob (**)", () => {
        it("matches any depth of path", () => {
            expect(matchUrl(glob("github.com/**"), "https://github.com/a/b/c")).toBe(true);
        });
    });
    describe("combined wildcard host + path", () => {
        it("matches subdomain and path together", () => {
            expect(matchUrl(glob("*.example.com/*"), "https://sub.example.com/path")).toBe(true);
        });
        it("matches bare hostname with path (Chrome match-pattern style)", () => {
            expect(matchUrl(glob("*.example.com/*"), "https://example.com/path")).toBe(true);
        });
        it("matches deep paths with trailing /*", () => {
            expect(matchUrl(glob("*.example.com/*"), "https://sub.example.com/a/b/c")).toBe(true);
        });
    });
    describe("YouTube URL matching (real-world)", () => {
        ...
        it("matches bare youtube.com", () => {
            expect(matchUrl(glob("*.youtube.com/*"), "https://youtube.com/watch?v=abc")).toBe(true);
        });
        it("matches youtube.com shorts", () => {
            expect(matchUrl(glob("*.youtube.com/*"), "https://www.youtube.com/shorts/abc123")).toBe(true);
        });
        it("matches youtube.com channel pages", () => {
            expect(matchUrl(glob("*.youtube.com/*"), "https://www.youtube.com/channel/UCxxx")).toBe(true);
        });
        it("matches m.youtube.com (mobile)", () => {
            expect(matchUrl(glob("*.youtube.com/*"), "https://m.youtube.com/watch?v=abc")).toBe(true);
        });
    ...
    describe("protocol-prefixed glob patterns", () => {
        ...
        it("strips https:// protocol prefix", () => {
            expect(matchUrl(glob("https://*.github.com/*"), "https://github.com/user")).toBe(true);
        });
        it("does NOT match when path doesn't satisfy the glob", () => {
            expect(matchUrl(glob("*://*.github.com/*/tree/*"), "https://github.com/")).toBe(false);
        });
        it("does NOT match a different domain with protocol prefix", () => {
            expect(matchUrl(glob("*://*.github.com/*"), "https://gitlab.com/page")).toBe(false);
        });
    });
    describe("question mark wildcard", () => {
        it("matches exactly one character", () => {
            expect(matchUrl(glob("example.com/ab?"), "https://example.com/abc")).toBe(true);
        });
        it("does NOT match zero characters", () => {
            expect(matchUrl(glob("example.com/ab?"), "https://example.com/ab")).toBe(false);
        });
    });
    it("returns false for an invalid URL", () => {
...
describe("matchUrl — regex matching", () => {
    it("matches a GitHub URL with a regex pattern", () => {
        expect(matchUrl(regex("^https://github\\.com/.*"), "https://github.com/user/repo")).toBe(true);
    });
    it("does NOT match a non-GitHub URL", () => {
        expect(matchUrl(regex("^https://github\\.com/.*"), "https://gitlab.com/user")).toBe(false);
    });
    ...
    it("supports regex flags implicitly (case sensitive by default)", () => {
        expect(matchUrl(regex("^https://GitHub\\.com"), "https://github.com")).toBe(false);
    });
});
// ---------------------------------------------------------------------------
// matchUrl — global
...
describe("matchUrl — edge cases", () => {
    ...
    it("handles URLs with ports", () => {
        expect(matchUrl(exact("localhost"), "http://localhost:3000/page")).toBe(true);
    });
    it("handles URLs with query strings (exact matches hostname only)", () => {
        expect(matchUrl(exact("example.com"), "https://example.com?foo=bar")).toBe(true);
    });
    it("handles URLs with fragments (exact matches hostname only)", () => {
        expect(matchUrl(exact("example.com"), "https://example.com#section")).toBe(true);
    });
    it("returns false for an unknown pattern type", () => {
        const unknown = { type: "unknown", value: "test" };

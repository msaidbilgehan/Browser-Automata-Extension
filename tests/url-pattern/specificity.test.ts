...
import {
  ...
  compareSpecificity,
  sortBySpecificity,
} from "@/shared/url-pattern/specificity";
...
function globNoWildcardHost(value: string): UrlPattern {
  return { type: "glob", value };
}

function globWildcardHost(value: string): UrlPattern {
...
describe("getSpecificity", () => {
  ...
  describe("host-wildcard detection with a protocol prefix", () => {
    ...
    it("ranks protocol-prefixed equivalents identically (the M9 regression)", () => {
      expect(getSpecificity(globWildcardHost("*://*.github.com/*"))).toBe(
        getSpecificity(globWildcardHost("https://*.github.com/*")),
      );
    });
  });
});
...
describe("compareSpecificity", () => {
  ...
  it("ranks exact > glob(no wildcard host)", () => {
    expect(
      compareSpecificity(exact("a.com"), globNoWildcardHost("a.com/*")),
    ).toBeGreaterThan(0);
  });
  ...
  it("ranks glob(no wildcard host) > glob(wildcard host)", () => {
    expect(
      compareSpecificity(
        globNoWildcardHost("a.com/*"),
        globWildcardHost("*.a.com"),
      ),
    ).toBeGreaterThan(0);
  });
  ...
  it("ranks glob(wildcard host) > regex", () => {
    expect(
      compareSpecificity(globWildcardHost("*.a.com"), regex("^https://a\\.com")),
    ).toBeGreaterThan(0);
  });

  it("ranks regex > global", () => {
    expect(
      compareSpecificity(regex("^https://a\\.com"), global()),
    ).toBeGreaterThan(0);
  });
});

...
import { Shield, Plus, ArrowLeft, Save, Trash2, X } from "lucide-react";
import type {
  NetworkRule,
  HeaderMod,
  NetworkRuleAction,
...
function NetworkRuleEditor({
  ...
  const [draft, setDraft] = useState<NetworkRule>(initial);
...
export function NetworkRulesView() {
  ...
  const ruleList = useMemo(
    () => Object.values(networkRules).sort((a, b) => a.name.localeCompare(b.name)),
    [networkRules],
  );
